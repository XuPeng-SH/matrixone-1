...
func JoinFilterEvalExpr(r, s *batch.Batch, rRow int, proc *process.Process, expr *plan.Expr) (*vector.Vector, error) {
	...
	case *plan.Expr_Col:
		if t.Col.RelPos == 0 {
			return r.Vecs[t.Col.ColPos].MakeConstRef(rRow, length, proc.Mp()), nil
		}
...
func JoinFilterEvalExprInBucket(r, s *batch.Batch, rRow, sRow int, proc *process.Process, expr *plan.Expr) (*vector.Vector, error) {
	...
	case *plan.Expr_Col:
		if t.Col.RelPos == 0 {
			return r.Vecs[t.Col.ColPos].MakeConstRef(rRow, 1, proc.Mp()), nil
		}
		return s.Vecs[t.Col.ColPos].MakeConstRef(sRow, 1, proc.Mp()), nil
	case *plan.Expr_F:
		var result *vector.Vector
