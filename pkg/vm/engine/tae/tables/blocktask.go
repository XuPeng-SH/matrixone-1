...
func (task *compactBlockTask) Execute() (err error) {
	...
	zmIdx := uint16(0)
	sfIdx := uint16(1)
	metas := common.NewEmptyIndicesMeta()
	...
	err = zoneMapWriter.Init(zmFile, common.Plain, uint16(schema.PrimaryKey), zmIdx)
	if err != nil {
		return err
	}
	...
	err = staticFilterWriter.Init(sfFile, common.Plain, uint16(schema.PrimaryKey), sfIdx)
	if err != nil {
		return err
	}
	...
	task.created = newBlk
	//logutil.Info(common.MockIndexBufferManager.String())
	return
}
